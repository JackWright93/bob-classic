...
export default function PlayerDetailPage() {
  ...
  const renderAvatar = (avatar: string | null, name: string) => {
    if (avatar && avatar.startsWith("http")) {
      return (
        <img src={avatar} alt={name} style={{ width: 72, height: 72, borderRadius: "50%", objectFit: "cover", border: `3px solid ${GOLD}`, boxShadow: `0 0 0 3px ${DARK_GREEN}` }} />
      );
    }
    if (avatar) {
      return (
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      roundsData.forEach((round) => {
        ...
        // Low NET round points — full points for all tied players
        const allPlayerTotals = players.map((p) => {
          ...
          const pHcp = calcRelativeHandicap(p.base_handicap ?? 0, lowestHandicap);
          ...
          return { playerId: p.id, total: netTotal };
        ...
        if (allPlayerTotals.length >= 1) {
          ...
          while (i < sorted.length) {
            ...
            const fullPts = pointsMap[i] ?? 0;
            if (fullPts > 0) {
              for (let k = i; k < j; k++) {
                if (sorted[k].playerId === playerId) {
                  const place = i === 0 ? "Low net" : i === 1 ? "2nd low net" : "3rd low net";
                  points += fullPts;
                  details.push(`🏌️ ${place} — tied (+${fullPts})`);
                }
              }
            }
            i = j;
